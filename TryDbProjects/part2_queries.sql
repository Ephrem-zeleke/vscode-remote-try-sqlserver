...
GO
-- Department Academic Performance Report with Advisor Count

SELECT 
    c.Department,
    COUNT(DISTINCT c.CourseID) AS NumberOfCourses,
    COUNT(e.EnrollmentID) AS TotalEnrollments,
    COUNT(DISTINCT a.AdvisorID) AS NumberOfAdvisors,
    AVG(CASE 
        WHEN e.Grade = 'A' THEN 4.0
        WHEN e.Grade = 'A-' THEN 3.7
        WHEN e.Grade = 'B+' THEN 3.3
...
FROM 
    Courses c
    LEFT JOIN Enrollments e ON c.CourseID = e.CourseID
    LEFT JOIN Advisors a ON c.Department = a.Department
WHERE 
    c.Credits >= 3 -- Example filter: only include 3+ credit courses
GROUP BY 
    c.Department
ORDER BY 
    AverageGPA DESC;
...
-- Enrollment date with highest average GPA
...
-- Advisor with the highest average GPA among their students
...
FROM 
    ...
    JOIN Enrollments e ON s.StudentID = e.StudentID
GROUP BY 
    a.AdvisorID, a.FirstName, a.LastName
ORDER BY 
    AvgGPA DESC;